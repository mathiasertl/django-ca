...
class HSMBackend(
    ...
    Access to HSMs is implemented using the PKCS11 protocol and `python-pkcs11
    <https://python-pkcs11.readthedocs.io/>`_.

    This backend has several mandatory options:

        * `pkcs11_library_path` specifies the path to the PKCS11 library (e.g.
    ...
    def __init__(
        ...
        super().__init__(
            alias, pkcs11_library_path=pkcs11_library_path, token=token, so_pin=so_pin, user_pin=user_pin
        )
    ...
    def session(self, so_pin: Optional[str], user_pin: Optional[str], rw: bool = False) -> Iterator[Session]:
        ...
        try:
            with SessionPool(self.pkcs11_library_path, self.token, so_pin, user_pin, rw=rw) as session:
                yield session

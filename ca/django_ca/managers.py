    ...
    def create_cert(
        ...
        **kwargs: Any,
        ...
        .. deprecated:: 1.18.0
           ...
           * The `csr_format` parameter will be removed (because CSRs must already be parsed).
           * Passing ``str`` or ``bytes`` for the `csr` parameter (pass a
             :py:class:`~cg:cryptography.x509.CertificateSigningRequest` instead).
           * ``str`` for the `profile` parameter (pass a :py:class:`~django_ca.profiles.Profile` instead).

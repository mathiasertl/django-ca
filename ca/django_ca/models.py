...
class X509CertMixin(DjangoCAModel):
    ...
    @property
    def algorithm(self) -> SignatureHashAlgorithm | None:
        """A shortcut for :py:attr:`~cg:cryptography.x509.Certificate.signature_hash_algorithm`."""
        return typing.cast(SignatureHashAlgorithm, self.pub.loaded.signature_hash_algorithm)
...
class CertificateAuthority(X509CertMixin):  # type: ignore[django-manager-missing]
    ...
    def generate_ocsp_key(
        ...
        algorithm will be the same as the one used in the CAs certificate. The subject of the certificate will
        be an empty name.
        ...
        .. versionchanged:: 2.5.0
           ...
           The `key_type`, `key_size`, `elliptic_curve`, `profile`, `algorithm` and `not_after` arguments
           were removed.

...
@shared_task
@transaction.atomic
def sign_certificate(  # pylint: disable=too-many-locals
    order_pk: int,
    csr: str,
    subject: List[SerializedObjectIdentifier],
    algorithm: Optional[str] = None,
    ...
    if algorithm is not None:
        parsed_algorithm = constants.HASH_ALGORITHM_TYPES[algorithm]()
    if expires is not None:
        parsed_expires = datetime.fromisoformat(expires)

    parsed_extensions: List[x509.Extension[x509.ExtensionType]] = []
    if extensions is not None:
        for extension_key, extension_data in extensions.items():
            parsed_extensions.append(parse_extension(extension_key, extension_data))
    ...
    certificate_obj = Certificate(ca=ca, profile=profile, autogenerated=autogenerated)

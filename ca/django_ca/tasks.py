...
def generate_ocsp_key(data: UseCertificateAuthorityTaskArgs) -> int | None:
    ...
    cert = ca.generate_ocsp_key(key_backend_options=key_backend_options, force=data.force)
    if cert is not None:
        return cert.pk
    return None
...
def cache_ocsp_response(data: CacheOCSPResponseTaskArgs) -> None:
    ...
    The task uses SHA-256 as the hash algorithm for the issuer hash inside the OCSP response. The
    :py:class:`~django_ca.views.GenericOCSPView` only serves cached responses for requests that also use
    SHA-256, so algorithm and cached data always match.
    """
    assert isinstance(data, CacheOCSPResponseTaskArgs)
...
def cache_ocsp_responses() -> None:
    ...
    Finds all non-expired, non-revoked certificates whose cached OCSP response will expire within
    :ref:`settings-ca-ocsp-response-cache-renewal` and regenerates them.  Certificates that have never had
    their response cached are also included.
    """
    if model_settings.CA_OCSP_RESPONSE_CACHE_EXPIRES is None:
        return  # Caching disabled — nothing to do.
...
def api_sign_certificate(data: ApiSignCertificateTaskArgs) -> int | None:
    ...
    try:
        certificate = ca.sign(
            key_backend_options,
            csr,
            subject=data.subject.cryptography,
            algorithm=algorithm,
    ...
    certificate_obj = Certificate(ca=ca, csr=csr, profile=data.profile, autogenerated=data.autogenerated)

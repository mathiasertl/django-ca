...
class CertificateAdminViewTestCase(
    ...
    def get_changelists(
        ...
        with self.freeze_time("ca_certs_expired"):
            yield (self.model.objects.all(), {"status": "all"})
            yield [
                self.new_certs["profile-client"],
                self.new_certs["profile-server"],
                self.new_certs["profile-webserver"],
                self.new_certs["profile-enduser"],
                self.new_certs["profile-ocsp"],
                self.new_certs["no-extensions"],
                self.new_certs["all-extensions"],
                self.new_certs["alt-extensions"],
            ], {}
            yield [
        ...
        with self.freeze_time("everything_valid"):
            ...
            self.new_certs["profile-ocsp"].autogenerated = True
            self.new_certs["profile-ocsp"].save()

            yield ([self.new_certs["profile-ocsp"]], {"auto": "auto"})
...
""",
    ...
    def test_unsupported_extensions(self) -> None:
        ...
        cert = self.new_certs["all-extensions"]

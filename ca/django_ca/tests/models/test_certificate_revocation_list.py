...
def test_full_crl(
    ...
    obj2: CertificateRevocationList = CertificateRevocationList.objects.create_certificate_revocation_list(
        usable_root, KEY_BACKEND_OPTIONS
    )
    assert obj2.ca == usable_root
    assert_crl_number(obj2, 1)
    assert [rev.serial_number for rev in obj2.loaded] == [

...
"""Test cache_ocsp_responses command."""
...
def test_command(settings: SettingsWrapper) -> None:
    ...
    with mock.patch(
        "django_ca.management.commands.cache_ocsp_responses.cache_ocsp_responses", autospec=True
    ) as task_mock:
        cmd("cache_ocsp_responses")
...
def test_command_with_celery(settings: SettingsWrapper) -> None:
    ...
    with mock_celery_task("django_ca.tasks.cache_ocsp_responses", mock.call((), {})):
        cmd("cache_ocsp_responses")

...
def test_with_no_parameters(usable_root: CertificateAuthority) -> None:
    ...
    generate_ocsp_key(GenerateOCSPKeyTaskArgs(serial=usable_root.serial))
...
def test_no_renewal_required(usable_root: CertificateAuthority) -> None:
    ...
    assert generate_ocsp_key(GenerateOCSPKeyTaskArgs(serial=usable_root.serial)) is not None
    assert generate_ocsp_key(GenerateOCSPKeyTaskArgs(serial=usable_root.serial)) is None
...
def test_with_wrong_password(usable_pwd: CertificateAuthority) -> None:
    ...
    message = GenerateOCSPKeyTaskArgs(serial=usable_pwd.serial, key_backend_options={"password": password})

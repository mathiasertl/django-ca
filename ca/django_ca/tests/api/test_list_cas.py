...
ExpectedResponse = List[Dict[str, Any]]
...
@pytest.fixture()
def expected_response(root_response: Dict[str, Any]) -> ExpectedResponse:
...
def test_list_view(api_client: Client, expected_response: ExpectedResponse) -> None:
...
@freeze_time(timestamps["everything_expired"])
def test_expired_filter(api_client: Client, expected_response: ExpectedResponse) -> None:
    """Test that expired CAs are excluded by default."""

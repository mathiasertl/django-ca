...
class GetCertTestCase(DjangoCAWithCSRTestCase):
    ...
    def test_basic(self):
        ...
        cert = Certificate.objects.init(
            self.ca, self.csr_pem, expires=720, algorithm='sha256', subjectAltName=['example.com'],
            **kwargs)
    ...
    def test_no_names(self):
        ...
        with self.assertRaises(ValueError):
            Certificate.objects.init(
                self.ca, self.csr_pem, expires=720, algorithm='sha256', subjectAltName=[],
                **kwargs)
        with self.assertRaises(ValueError):
            Certificate.objects.init(
                self.ca, self.csr_pem, expires=720, algorithm='sha256', subjectAltName=None,
                **kwargs)
    ...
    def test_cn_in_san(self):
        ...
        cert = Certificate.objects.init(self.ca, self.csr_pem, expires=720, algorithm='sha256',
                                        **kwargs)
        self.assertEqual(self.get_subject(cert)['CN'], 'cn.example.com')
        self.assertIn('subjectAltName', self.get_extensions(cert))
    ...
    def test_crl(self):
        ...
        kwargs = get_cert_profile_kwargs()
        cert = Certificate.objects.init(
            ca, self.csr_pem, expires=720, algorithm='sha256', subjectAltName=['example.com'],
            **kwargs)

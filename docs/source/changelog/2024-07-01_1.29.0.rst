...
* **BACKWARDS INCOMPATIBLE:** Removed ``django_ca.utils.parse_hash_algorithm()``, deprecated since
  1.25.0. Use :py:attr:`standard hash algorithm names <django_ca.typehints.SignatureHashAlgorithmName>`
  instead.
...
* Add literal typehints for extension keys under :py:attr:`~django_ca.typehints.ExtensionKey` and
  :py:attr:`~django_ca.typehints.CertificateExtensionKey` to improve type hinting.
